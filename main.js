...
const app = {
    ...
    openArena: function(type, customId = null) {
        if (type === 'coin') {
            ...
            this.currentDie = { type: 'die', name: 'D6', faces: [1,2,3,4,5,6] };
        } else if (type === 'd20') {
            this.currentDie = { type: 'die', name: 'D20', faces: Array.from({length:20}, (_,i)=>i+1) };
        } else if (type === 'custom') {
            const saved = JSON.parse(localStorage.getItem('minimalDice') || '[]');
            const found = saved.find(d => d.id === customId);
            if(found) this.currentDie = { ...found, type: 'die' };
        }
    ...
    rollNow: function() {
        ...
        const faces = this.currentDie.faces;
    ...
    renderSaved: function() {
        ...
        list.forEach(die => {
            ...
            card.className = 'card';
            card.onclick = () => this.openArena('custom', die.id);
            card.innerHTML = `
                <button class="card-del" onclick="app.deleteDie(event, ${die.id})"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
                <div class="card-icon svg-icon">${ICONS.custom}</div>
                <div class="card-info"><span class="card-title">${die.name}</span><span style="font-size:0.7rem; opacity:0.7">${die.faces.length} Faces</span></div>
            `;
